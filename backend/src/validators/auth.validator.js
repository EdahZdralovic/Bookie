...
const isId = (value) => typeof value === 'string' && /^[1-9]\d{0,9}$/.test(value) && Number(value) <= 2147483647;
...
function validateLogin(body = {}) {
  ...
  else if (Buffer.byteLength(password, 'utf8') > AUTH.PASSWORD.MAX_BYTES) errors.password = EXCEPTIONS.PASSWORD_TOO_LONG;
...
function validateRegistration(body = {}) {
  ...
  for (const field of ['firstName', 'lastName']) {
    ...
    else if (!/^[\p{L}\p{M}][\p{L}\p{M} .'-]*$/u.test(data[field])) errors[field] = EXCEPTIONS.INVALID_NAME;
  }
  data.password = text(body.password);
  if (!data.password) errors.password = EXCEPTIONS.REQUIRED_FIELD;
  else if (Buffer.byteLength(data.password, 'utf8') > AUTH.PASSWORD.MAX_BYTES) errors.password = EXCEPTIONS.PASSWORD_TOO_LONG;
  else if (data.password.length < AUTH.PASSWORD.MIN_LENGTH || !/\p{Lu}/u.test(data.password) || !/[0-9]/.test(data.password)) errors.password = EXCEPTIONS.WEAK_PASSWORD;
  if (!text(body.repeatPassword)) errors.repeatPassword = EXCEPTIONS.REQUIRED_FIELD;
  else if (data.password !== body.repeatPassword) errors.repeatPassword = EXCEPTIONS.PASSWORD_MISMATCH;
  data.role = body.role;
  ...
  for (const field of ['genreIds', 'languageIds']) {
    ...
    if (ids.length === 0) errors[field] = field === 'genreIds' ? EXCEPTIONS.GENRES_REQUIRED : EXCEPTIONS.LANGUAGES_REQUIRED;
    else if (ids.length > AUTH.MAX_INTERESTS || !ids.every(isId)) errors[field] = EXCEPTIONS.INVALID_INTERESTS;
